...
```
Property admin                    Your backend                  Stellar / Soroban
      │                                 │                              │
      │── POST /api/properties ────────▶│── deployRealEstate() ───────▶│ token deployed
      │── POST /api/investors ─────────▶│── registry.mint() ──────────▶│ shares minted
      │                                 │                              │
      │  (tenant moves in) ────────────▶│── updateOccupancyStatus() ──▶│ status updated
      │                                 │                              │
      │  (quarterly) ───────────────────│  compute payouts off-chain
      │                                 │  pay each holder via bank/mobile-money
      │                                 │── declareRentalDistribution()▶ on-chain audit record
```
