...
```
Diaspora buyer (payer)            Your backend                 Contractor (payee)
       │                                │                              │
       │                                │── deployEscrow() ───────────▶│ escrow deployed (arbiter set)
       │                                │                              │
       │── fund(0) ────────────────────▶│                              │ (buyer's own signer)
       │                                │◀───── markDelivered(0) ──────│ (contractor's own signer)
       │── approveMilestone(0) ────────▶│                              │ funds released
       │                                │                              │
       │  (disagreement) ── raiseDispute(0) ──▶ arbiter.resolveDispute(0, releaseToPayee)
       │                                │                              │
       │  (buyer unresponsive) ─────────┼──────── claimTimelockRelease(0) ◀── (after timelock elapses)
```
