...
stateDiagram-v2
    ...
    note left of PENDING
        payer: fund(milestoneId)
        (toggles the separate `funded`
        flag — not a status transition)
    end note

    PENDING --> DELIVERED: payee — markDelivered()

    DELIVERED --> RELEASED: payer — approveMilestone()
    DELIVERED --> DISPUTED: payer — raiseDispute()
    DELIVERED --> RELEASED: payee — claimTimelockRelease()\n(after timelockDurationSeconds)

    DISPUTED --> RELEASED: arbiter — resolveDispute(true)
    DISPUTED --> REFUNDED: arbiter — resolveDispute(false)
