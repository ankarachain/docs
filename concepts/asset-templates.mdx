...
NFT templates share the same metadata shapes as their fractional counterparts (e.g. `FarmlandNFTMetadata` mirrors `FarmlandMetadata`), with the addition of a `surveyReportHash` field on `FarmlandNFTMetadata`. You can also link an NFT deed to a corresponding ERC-20 fractional token using `assetLinkToERC20()`.
