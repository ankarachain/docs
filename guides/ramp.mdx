---
title: "Fiat On-Ramp and Off-Ramp with Ankara Chain"
...
To use MoonPay, pass a `RampProviderSelection` with `provider: "moonpay"` to `createRampProvider()`:

```typescript
import { createRampProvider, RampManager } from "@ankarachain/sdk";
...
Use `createRampProvider()` for declarative provider selection from a config object — useful for reading provider settings from a database or environment variables without importing provider classes directly:

```typescript
import { createRampProvider } from "@ankarachain/sdk";
...
const provider = createRampProvider({
  ...
  // Note: StellarAnchorProvider needs a signer — use RampManager.connectAnchor()
  // for Stellar anchors to wire in the signer; createRampProvider is best for
  // "manual" and "moonpay" which don't need one at construction time.
});
```
...
```typescript
import { RampManager, ManualRampProvider, createRampProvider } from "@ankarachain/sdk";
