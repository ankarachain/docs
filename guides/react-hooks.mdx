---
title: "React Hooks for Ankara Chain Token Data"
...
For Stellar, pass a `StellarAnkaraChainConfig` with `stellarSigner` instead of `stellarSecretKey`. A `stellarSigner` matches Freighter's API shape exactly, so you can pass the Freighter global object directly:
